...
TARGET_NAME := multiap_controller
...
EXTRA_FLAGS = -DOPENWRT -DMAP_MGMT_IPC
endif

#Select roaming engine
ifeq ($(ROAMING_ENGINE), conductor)
TARGET_LIBRARIES += -lwificonductor
EXTRA_FLAGS      += -DEXT_ROAMING_ENGINE -DEXT_ROAMING_ENGINE_CONDUCTOR
endif
...
CFLAGS+= -DCONTROLLER_VERSION=\"$(CONTROLLER_VERSION)\"
